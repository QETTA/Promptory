...
export function ChannelIntakeCard({
  ...
  return (
    <Card variant="heroBright" className={cn("overflow-hidden p-4 sm:p-5", className)}>
      <div className="ui-panel-info p-4">
        <div className="flex items-start justify-between gap-3">
          ...
          <Badge variant="neutral" className="bg-white px-3 py-1 text-[11px] font-semibold text-[var(--brand-700)]">
        ...
        <div className="mt-4 grid gap-2 sm:grid-cols-2">
          {supportedChannelCatalog.map((channel) => (
            <div
              key={channel.kind}
              className="ui-panel-muted bg-white/86 px-3 py-2 text-xs font-semibold text-[var(--slate-700)] shadow-[0_10px_20px_-22px_rgba(15,23,42,0.22)]"
            >
              {channel.label}
            </div>
          ))}
        ...
        <div className="ui-panel-elevated rounded-3xl p-2">
          <div className="grid gap-2 sm:grid-cols-[minmax(0,1fr)_132px]">
            <Input
              ...
              className="h-12 border-transparent shadow-none focus:border-[var(--brand-200)] focus:ring-[var(--color-focus-ring)]"
            />
