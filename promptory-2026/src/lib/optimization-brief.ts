...
export const optimizationConversionOptions = [
  { value: "purchase", label: "바로 구매", description: "상세 화면이나 스토어에서 바로 구매로 이어집니다." },
...
function getChannelBenchmark(kind: ParsedChannelInputSuccess["kind"]) {
  switch (kind) {
    ...
    case "smartstore":
      return "같은 고객층을 노리는 스토어에서 대표 판매 카드, 후기, 신뢰 요소가 어떻게 묶이는지";
    default:
...
function getBaseModules(kind: ParsedChannelInputSuccess["kind"], inquiryFlow = false): RecommendedModule[] {
  switch (kind) {
    case "youtube":
      return [
        { title: "첫 문장 흐름", body: "영상 제목, 썸네일 약속, 첫 30초 오프닝 문장을 채널 톤에 맞게 한 흐름으로 묶습니다." },
        { title: "설명란 전환 흐름", body: "영상 설명, 고정 댓글, 링크 허브 행동 유도 문장을 한 세트의 전환 흐름으로 최적화합니다." },
        { title: "댓글 전환 흐름", body: "댓글 유도, 문의 유도, 구매 유도를 단계별 답변 흐름으로 정리합니다." },
      ];
    case "blog":
      return [
        { title: "제목 흐름", body: "제목, 도입, 검색형 약속을 한 세트로 조정해 첫 클릭과 체류를 올립니다." },
        { title: "신뢰 근거 묶음", body: "본문 중간 근거, 비교 포인트, 사례 블록을 구조화해 한 레이어로 정리합니다." },
        { title: "말미 전환 흐름", body: "말미 행동 유도 문장, 링크 문장, 재방문 유도 문장을 한 흐름으로 정리합니다." },
      ];
    case "coupang":
      if (inquiryFlow) {
        return [
          { title: "문의 시작 흐름", body: "첫 화면, 대표 혜택, 문의 버튼 앞 문장을 묶어 지금 바로 물어볼 이유를 만듭니다." },
          { title: "응답 신뢰 묶음", body: "후기, 운영 원칙, 응답 기준, 안심 문구를 한 레이어로 묶어 상담 시작 장벽을 낮춥니다." },
          { title: "질문 분류 흐름", body: "어떤 고객이 무엇을 물어보면 되는지, 상담 전 먼저 걸러 줄 질문 흐름으로 정리합니다." },
        ];
      }

      return [
        { title: "제안 정리 묶음", body: "상품명 첫 문장, 대표 혜택, 첫 화면 가치 제안을 한 레이어로 정리합니다." },
        { title: "신뢰 증명 묶음", body: "리뷰 요약, 품질 근거, FAQ, 반품/보증 문구를 한 레이어로 만듭니다." },
        { title: "구매 유도 흐름", body: "비교 고객이 바로 결제하도록 행동 유도 문장과 순서를 조정합니다." },
      ];
    case "smartstore":
      if (inquiryFlow) {
        return [
          { title: "톡톡 시작 흐름", body: "스토어 소개, 대표 상품 첫 화면, 톡톡 안내 문장을 묶어 말 걸기 쉬운 첫 흐름을 만듭니다." },
          { title: "운영 신뢰 묶음", body: "후기, 응답 톤, 운영 기준, 안심 문구를 같은 레이어로 정리해 문의 시작을 돕습니다." },
          { title: "안내 분기 흐름", body: "어떤 고객에게 어떤 답을 먼저 줄지, 문의 전 분기 문장과 답변 방향을 정리합니다." },
        ];
      }

      return [
        { title: "스토어 톤 정리", body: "스토어 소개, 대표 상품 톤, 브랜드 약속을 같은 말투로 정리합니다." },
        { title: "후기 배치 묶음", body: "후기, 묶음 제안, 비교 포인트를 구매 흐름에 맞게 배치합니다." },
        { title: "전환 흐름", body: "스토어 탐색에서 대표 상품 클릭과 구매 행동까지 이어지는 문장을 만듭니다." },
      ];
    default:
      return [
        { title: "기본 진단 묶음", body: "지원 채널 확인과 핵심 surface 파악을 먼저 정리합니다." },
        { title: "수동 검토 기준", body: "수동 검토 기준과 필수 진단 포인트를 분리해 보여 줍니다." },
        { title: "기본 행동 유도 흐름", body: "지원 범위 밖에서도 공통 행동 유도 문장과 핵심 제안은 먼저 정리합니다." },
      ];
  }
}
...
function applyPainOverlay(
  ...
  if (pain === "manual_overload") {
    result.unshift({
      title: inquiryFlow ? "문의 운영 자동화 묶음" : "운영 자동화 묶음",
      body: inquiryFlow
        ? "반복되는 문의 시작, 안심, 답변 마무리 문구를 재사용 가능한 상담 실행 묶음으로 분해합니다."
        : "반복되는 채널 운영 문구를 재사용 가능한 플러그인과 실행 묶음으로 분해합니다.",
    });
  } else if (pain === "low_trust") {
    result.unshift({
      title: inquiryFlow ? "문의 안심 묶음" : "신뢰 증명 묶음",
      body: inquiryFlow
        ? "후기, 응답 기준, 운영 신뢰를 채널 surface마다 재사용 가능한 상담 안심 묶음으로 만듭니다."
        : "후기, 비교 근거, 안심 포인트를 채널 surface마다 재사용 가능한 증명 묶음으로 만듭니다.",
    });
  } else if (pain === "weak_cta") {
    result.unshift({
      title: inquiryFlow ? "문의 시작 흐름" : "행동 유도 흐름",
      body: inquiryFlow
        ? "DM, 톡톡, 채팅, 문의 버튼 앞뒤 문장을 즉시 적용 가능한 상담 시작 흐름으로 분해합니다."
        : "댓글, 링크, DM, 구매 버튼 앞뒤 문장을 즉시 적용 가능한 행동 유도 흐름으로 분해합니다.",
    });
  } else if (pain === "weak_hook") {
    result.unshift({
      title: inquiryFlow ? "첫 상담 가치 정리" : "첫 인상 보강 흐름",
      body: inquiryFlow
        ? "첫 문장, 첫 화면, 첫 문의를 담당하는 상담 가치 흐름을 채널별로 분해해 추천합니다."
        : "첫 문장, 첫 화면, 첫 클릭을 담당하는 첫 인상 흐름을 채널별로 분해해 추천합니다.",
    });
  } else if (pain === "unclear_offer") {
    result.unshift({
      title: inquiryFlow ? "질문 분류 흐름" : "제안 선명화 묶음",
      body: inquiryFlow
        ? "누가 무엇을 물어보면 되는지 읽히게 만드는 질문 분류 흐름을 먼저 추천합니다."
        : "무엇을 주고 누구에게 맞는지 읽히게 만드는 제안 정리 묶음을 먼저 추천합니다.",
    });
  }
  ...
  if (goal === "repeat_visit") {
    result.push({
      title: "재방문 루프",
      body: "시리즈 재방문, 저장, 팔로우 이후 재유입을 만드는 반복 루프를 추가합니다.",
    });
  }
      ...
      if (salesChannel) {
        if (salesInquiryFlow) {
          return {
            ...
            diagnosisBody: `문의 시작 문장, 안심 문장, 답변 마무리 문장을 매번 새로 쓰지 말고 같은 실행 묶음으로 재사용하는 편이 좋습니다. ${snapshot.surfaceHint}를 문의 흐름 단위로 쪼개면 운영 부담이 빠르게 줄어듭니다. ${browserClause}`.trim(),
          };
        }

        return {
          diagnosisHeadline: "상품마다 따로 손보는 운영 방식이 판매 속도를 묶고 있습니다",
          diagnosisBody: `대표 혜택 문장, 신뢰 문장, 구매 직전 문장을 매번 새로 쓰지 말고 같은 실행 묶음으로 재사용하는 편이 좋습니다. ${snapshot.surfaceHint}를 실행 흐름 단위로 쪼개면 운영 부담이 빠르게 줄어듭니다. ${browserClause}`.trim(),
        };
      }

      return {
        diagnosisHeadline: "채널마다 따로 손보는 운영 방식이 성장을 묶고 있습니다",
        diagnosisBody: `지금 가장 큰 문제는 한 번 만든 문장을 여러 채널에 재사용하지 못하는 점으로 보입니다. ${snapshot.surfaceHint}를 실행 흐름 단위로 쪼개 재사용 구조를 먼저 만드는 편이 좋습니다. ${browserClause}`.trim(),
      };
    default:
      return {
  ...
  if (!brief.hasAnyInput) {
    return [
      ...
      },
      {
        title: "어디에 먼저 붙이나",
        emphasis: placement,
        body: "가장 먼저 보이는 위치부터 확인해야 이후 실행 묶음 추천이 과장되지 않습니다.",
      },
    ...
    case "manual_overload":
      if (salesInquiryFlow) {
        return [
          {
            title: "바꿀 문장",
            emphasis: "반복되는 문의 응답 문장을 흐름 단위로 쪼개 재사용해야 합니다",
          ...
          },
          {
            title: "어디에 먼저 붙이나",
            emphasis: placement,
            body: "반복해서 쓰는 문의 시작 문장부터 같은 실행 흐름을 재사용하는 구조로 정리하면 운영 부담이 빠르게 줄어듭니다.",
          },
        ] satisfies BriefResultCard[];
      }
      ...
      return [
        {
          title: "바꿀 문장",
          emphasis: "반복되는 문장을 흐름 단위로 쪼개 재사용해야 합니다",
        ...
        },
        {
          title: "어디에 먼저 붙이나",
          emphasis: placement,
          body: "반복해서 쓰는 대표 위치부터 같은 실행 흐름을 재사용하는 구조로 정리하면 운영 부담이 빠르게 줄어듭니다.",
        },
      ] satisfies BriefResultCard[];
    default:
      return [
  ...
  return {
    ...
    browserReviewCards: [
      ...
      },
      {
        ...
        body: "브라우저로 다시 본 뒤에는 이 기준으로 어떤 실행 묶음을 먼저 적용할지 바로 결정합니다.",
      },
    ] satisfies BriefResultCard[],
    browserReviewBody:
...
function getChannelHookDraft({
  ...
  if (kind === "coupang" || kind === "smartstore") {
    if (inquiryFlow) {
      ...
      if (pain === "manual_overload") {
        return {
          draft: `${audienceLead} 문의 시작 문장을 상품마다 다시 쓰지 않게, 첫 화면 문장을 같은 틀로 재사용해 보세요.`,
    ...
    if (pain === "manual_overload") {
      return {
        draft: `${audienceLead} 대표 혜택 문장을 상품마다 다시 쓰지 않게, 첫 화면 문장을 같은 틀로 재사용해 보세요.`,
...
export function buildOptimizationBriefSummary({
  ...
  const nextStepSignals = [
    ...
    "다음 결과 화면에서는 한 번에 붙여 넣는 긴 프롬프트 대신, 바로 적용할 실행 묶음으로 결과를 나눠 보여 줍니다.",
  ];
  ...
  if (!brief.hasAnyInput) {
    ...
    directionPoints = [
      ...
      "지금 확인한 주요 위치를 기준으로 첫 질문부터 좁혀 가면, 결과를 과장하지 않고 바로 쓸 실행 묶음 추천으로 이어질 수 있습니다.",
    ...
    directionPoints = [
      ...
      `${surfaceAction}를 중심으로 ${brief.pain ? painAction : "현재 pain"}을 더 분명히 고르면 다음 단계의 실행 묶음 추천 정밀도가 올라갑니다.`,
      brief.constraint
        ? `제약 조건: ${brief.constraint}`
