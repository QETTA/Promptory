...
test("optimization brief summary sharpens sales copy for smartstore even when live read is unavailable", () => {
  ...
  assert.match(summary.recommendedModules[0]?.title ?? "", /신뢰 증명 묶음|신뢰 증명/);
...
test("optimization brief summary pivots completed cards toward inquiry flow for sales channels", () => {
  ...
  assert.match(summary.recommendedModules[0]?.title ?? "", /문의 시작 흐름/);
  assert.match(summary.recommendedModules[1]?.title ?? "", /문의 시작 흐름|응답 신뢰 묶음|질문 분류 흐름/);
  assert.equal(summary.recommendedModules.some((module) => /구매 유도 흐름|전환 흐름/.test(module.title)), false);
});
