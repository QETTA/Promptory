...
export function AuthForm({ mode, redirectTo }: AuthFormProps) {
  ...
  if (mode === "signup" && pendingVerificationEmail) {
    return (
      <Card variant="strong" className={styles.shell}>
        <p className={`section-kicker ${styles.eyebrow}`}>Email verification</p>
        ...
        <p className={styles.body}>
          <span className={styles.verificationEmail}>{pendingVerificationEmail}</span>로
        ...
        <div className={styles.verificationActions}>
          <Button type="button" size="lg" className={styles.submit} onClick={resendVerification} disabled={isResendingVerification}>
            {isResendingVerification ? "재전송 중..." : "인증 메일 다시 보내기"}
          </Button>
          <Link href={buildLoginHref(destination)} className={`${styles.backLink} ${styles.centerLink}`}>
            로그인 화면으로 이동
          </Link>
        </div>
      </Card>
  ...
  return (
    <Card variant="strong" className={styles.shell}>
      <p className={`section-kicker ${styles.eyebrow}`}>
      ...
      <p className={styles.body}>
        {mode === "login"
          ...
          : "계정을 만들면 인증 메일로 안전하게 시작하고, URL 진단 저장과 실행 워크플로를 같은 계정으로 관리할 수 있습니다."}
      </p>
      ...
      <form className={styles.form} onSubmit={handleSubmit(onSubmit)}>
        {mode === "signup" ? (
          <div>
            ...
            {"displayName" in errors && errors.displayName ? (
              <p className={styles.fieldError}>이름은 2자 이상 입력해 주세요.</p>
            ) : null}
          </div>
        ) : null}
        ...
        <div>
          ...
          {errors.email ? (
            <p className={styles.fieldError}>올바른 이메일 주소를 입력해 주세요.</p>
          ) : null}
        </div>
        ...
        <Button className={styles.submit} size="lg" type="submit" disabled={isSubmitting}>
      ...
      <div className={styles.switch}>
        {mode === "login" ? (
          <>
            아직 계정이 없나요?{" "}
            <Link href={buildSignupHref(destination)} className={styles.switchLink}>
              회원가입
            </Link>
          </>
        ) : (
          <>
            이미 계정이 있나요?{" "}
            <Link href={buildLoginHref(destination)} className={styles.switchLink}>
              로그인
            </Link>
          </>
        )}
